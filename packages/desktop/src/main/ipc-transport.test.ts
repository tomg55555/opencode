  ...
  test("keeps multiple renderer ports independent", async () => {
    const handlers = TestRpcs.toLayer(
      Effect.gen(function* () {
        ...
        return TestRpcs.of({
          "test.focused": (_args, context) => Effect.succeed(handoff.sender(context.client.id)?.id === 1),
          "test.blob.put": ([data]) => Effect.succeed([...data].join(",")),
          "test.blob.get": () => Effect.succeed(new Uint8Array([3, 1, 4])),
          "test.events": () => Stream.make("session.new"),
        })
      }),
    )
    ...
    expect(await firstEvent(firstClient)).toBe("session.new")
...
const TestRpcs = RpcGroup.make(
  Rpc.make("test.focused", { payload: Schema.Tuple([]), success: Schema.Boolean }),
  Rpc.make("test.blob.put", { payload: Schema.Tuple([Schema.Uint8Array]), success: Schema.String }),
  Rpc.make("test.blob.get", { payload: Schema.Tuple([]), success: Schema.Uint8Array }),
  Rpc.make("test.events", { payload: Schema.Tuple([]), success: Schema.String, stream: true }),
)
